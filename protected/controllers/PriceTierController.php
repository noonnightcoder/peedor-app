...
class PriceTierController extends Controller
{
    ...
    public function actionAdmin()
    {
        ...
        $page_size = CHtml::dropDownList(
            'pageSize',
            Yii::app()->user->getState('pricetier_page_size', Common::defaultPageSize()),
            Common::arrayFactory('page_size'),
            array('class' => 'change-pagesize')
        );

        $data['model'] = $model;
        $data['grid_id'] = 'price-tier-grid';
        $data['main_div_id'] = strtolower(get_class($model)) . '_cart';
        $data['page_size'] = $page_size;
        $data['create_permission'] = 'item.create';
        $data['modal_header'] = Yii::t('app','New Price Tier');

        $data['grid_columns'] = PriceTier::getPriceTierColumn();
